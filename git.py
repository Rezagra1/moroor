...
#stash
هنگامي که بخواهيم تغييرات اعمال شده را کاميتت نکنيم و به شاخه ديگر برويم
و تغييرات را با خودمان نبريم از استش استفاده ميکنيم
با اين کار تغييرات در يک جاي موقت ذخيره ميشوند

git stash : ذخيره تغييرات و تميز کردن درخت کاري
git stash list : نمايش ليست استش ها
git stash drop stashname : خذف استش با اسم
git stahs save "message" : ذخيره استش با پيام دلخواه
git stash show stashname : مشاهده تغييرات ذخيره شده داخل استش ها
git stash show -p stashname : مشاهده تغييرات ذخيره شده داخل استش با جزييات بيشتر

git stash pop stshname : برگرداندن استش به به شاخه جاري و حذف از ليست استش ها
git stash apply stashname : برگرداندن استش به شاخه جاري و باقي مانده در ليست اشتش ها

#gitignore
براي عدم رديابي فايل هاي در گيت استفاده ميشود
touch .gitignore : ساخت فايل گيت ايقنور
داخل فايل گيت ايقنور مواردي که ميخواهيم ترک نشوند به صورت دستي اضافه ميکنيم
example:
    node/ "يعني پوشه نود ترک نشود اسلش به معني پوشهاست"
    *.txt  "تمامي فايل ها با پسنوند تکست ترک نشوند"
    !a.txt "استثنا اين فايل تکس ترک شود"
جهت آن ترک کردن دايرکتوري که از قبل ترک شده است :
